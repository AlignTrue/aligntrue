...
Test pushing rules to remote git repositories for backup:
...
# Test backup status with no config
aligntrue backup status
# Expected: "No backup configured"

# Test backup push with no config
aligntrue backup push
# Expected: error about no backup configured

# Configure a default remote backup (use local git repo for testing)
git init --bare /tmp/test-backup-remote.git
cat >> .aligntrue/config.yaml <<'EOF'
backup:
  default:
    url: /tmp/test-backup-remote.git
    branch: main
    auto: true
EOF

# Test backup status with config
aligntrue backup status
# Expected: shows default backup with file count

# Test manual backup push
aligntrue backup push
# Expected: success message with file count

# Verify files were pushed
git clone /tmp/test-backup-remote.git /tmp/verify-backup
ls /tmp/verify-backup/.aligntrue/rules/
# Expected: typescript.md, testing.md, guides/react.md (preserves structure)

# Test auto backup during sync
echo "New content" >> .aligntrue/rules/typescript.md
aligntrue sync
# Expected: sync completes AND backup push happens automatically

# Test backup with multiple destinations
git init --bare /tmp/test-backup-oss.git
cat > .aligntrue/config.yaml <<'EOF'
backup:
  default:
    url: /tmp/test-backup-remote.git
...
aligntrue backup push
# Expected: files assigned to appropriate backups

aligntrue backup status
# Expected: shows both backups with their file assignments
```

**Expected:**

- `backup status` shows configured backups and file assignments
- `backup push` manually pushes to all configured backups
- Files are pushed preserving directory structure
- `auto: true` triggers backup during `aligntrue sync`
- Multiple backups supported with glob-based file assignment
- Default backup catches unassigned files
- Files matching multiple backups go to first match with warning
...
# Expected:
# - Source added to config with gitignore: true
# - "Run 'aligntrue sync' to pull rules" message

# Verify config has gitignore flag
grep "gitignore: true" .aligntrue/config.yaml || echo "FAIL: gitignore flag not set"
...
- SSH URLs (`git@`, `ssh://`) auto-detected as gitignore sources
- Gitignore source rules are auto-added to .gitignore (both source and exports)
- Auto-sync happens after import (unless `--no-sync`)
- `--link` with SSH source sets `gitignore: true` in config
- Frontmatter `gitignore: true` overrides source settings
- Enhanced tips section shows gitignore-aware guidance
...
Backup & Recovery:
- aligntrue backup <create|list|restore|cleanup|push|status|setup> [options]
- aligntrue revert [--timestamp <timestamp>] [--yes]
