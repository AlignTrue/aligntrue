...
function parseCliArgs(): CliArgs {
  ...
  if (positionals.length < 2) {
    console.error(
      "Usage: run-checks <align-file> <target-dir> [--allow-exec] [--format sarif|json|text]",
    );
    process.exit(2);
  }
  ...
  return {
    ...
    format: ((values["format"] as string) || "text") as
      | "sarif"
      | "json"
      | "text",
  };
...
async function main() {
  ...
  // Run checks
  console.error(
    `Running ${alignPack.rules.length} checks from ${alignPack.id}...`,
  );
  if (args.allowExec) {
