 ...
 */
export function generateMarkdown(ir: IRDocument, options?: GenerateOptions): string {
  const opts = options || {}
  
  // Extract metadata if preserving
  const meta: MarkdownMetadata | undefined = (opts.preserveMetadata && ir._markdown_meta) 
    ? ir._markdown_meta 
    : undefined
  ...
  const indentSize = opts.indentSize 
    || meta?.whitespace_style?.indent_size 
    || 2
  
  const useSpaces = meta?.whitespace_style?.indent === 'tabs' ? false : true
  
  const lineEnding = opts.lineEndings 
    || meta?.whitespace_style?.line_endings 
    || 'lf'
