...
describe('Markdown Round-Trip', () => {
  it('golden repo round-trip is semantically identical', () => {
    ...
    const goldenRepoPath = join(process.cwd(), '../../examples/golden-repo/.aligntrue/rules.md')
    ...
    const regenerated = generateMarkdown(irResult.document!, { preserveMetadata: true })
    ...
    expect(irResult2.document?.spec_version).toBe(irResult.document?.spec_version)
    expect(irResult2.document?.rules).toHaveLength(irResult.document!.rules.length)
...
`
    ...
    expect(irResult.document?._markdown_meta?.guidance_position).toBe('before-block')
...
`
    ...
    expect(irResult.document?._markdown_meta?.whitespace_style?.indent_size).toBe(4)
