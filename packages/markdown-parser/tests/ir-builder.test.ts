...
rules: []`,
    ...
    expect(result.errors).toEqual([]);
    expect(result.document?.guidance).toBe(
      "This is guidance from markdown prose.",
    );
  });
...
rules:
  - id: testing.require.tests
    ...
    expect(result.document?._markdown_meta?.original_structure).toBe(
      "single-block",
    );
    expect(result.document?._markdown_meta?.header_prefix).toBe(
      "# AlignTrue Rules",
    );
    expect(result.document?._markdown_meta?.whitespace_style?.indent).toBe(
      "spaces",
    );
    expect(result.document?._markdown_meta?.whitespace_style?.indent_size).toBe(
      2,
    );
    expect(
      result.document?._markdown_meta?.whitespace_style?.line_endings,
    ).toBe("lf");
  });
