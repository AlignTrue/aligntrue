  ...
  const res = await fetchImpl(
    `https://api.github.com/app/installations/${params.installationId}/access_tokens`,
    {
      ...
      headers: {
        ...
        "User-Agent": "aligntrue-app-infra",
      },
    },
  );
...
async function getCachedToken(
  ...
  const cached = await (redis ?? ensureRedis()).get<CachedToken>(
    TOKEN_CACHE_KEY,
  );
...
async function setCachedToken(
  ...
  await (redis ?? ensureRedis()).set(TOKEN_CACHE_KEY, token, {
    ex: ttlSeconds,
  });
}
...
function loadConfig(overrides?: {
  ...
  const appId = overrides?.appId ?? process.env["GITHUB_APP_ID"];
  const installationId =
    overrides?.installationId ?? process.env["GITHUB_APP_INSTALLATION_ID"];
  const privateKeyEnv =
    overrides?.privateKey ?? process.env["GITHUB_APP_PRIVATE_KEY"];
...
export async function getGitHubAppToken(options?: {
  ...
  if (!hasGitHubAppConfig() && !options?.appId) {
    inMemoryToken = null;
    return null;
  }
  ...
  const cached = await getCachedToken(options?.redis);
  ...
  await setCachedToken(fresh, options?.redis);
  return fresh.token;
}
