...
import type { OgContent } from "../src/og/generate.js";
...
describe("ensureOgImage", () => {
  ...
  it("returns null when blob env missing", async () => {
    delete process.env.BLOB_READ_WRITE_TOKEN;
    const { ensureOgImage } = await import("../src/og/service.js");
    const result = await ensureOgImage({ id: "demo-1", content });
