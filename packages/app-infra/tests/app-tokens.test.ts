...
async function importModule() {
  vi.resetModules();
  return await import("../src/github/app-tokens.js");
}
...
describe("getGitHubAppToken", () => {
  ...
  it("returns a token and caches in memory", async () => {
    ...
    const { getGitHubAppToken } = await importModule();
  ...
  it("reads from Redis cache across module reloads", async () => {
    ...
    const { getGitHubAppToken: reloaded } = await importModule();
...
describe("getAuthToken", () => {
  ...
  it("prefers GitHub App token when configured", async () => {
    ...
    const { getAuthToken } = await importModule();
