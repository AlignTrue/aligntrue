import { NOTE_EVENT_TYPES, type NoteEvent } from "./events.js";
...
export function reduceEvent(
  ...
  switch (event.event_type) {
    case NOTE_EVENT_TYPES.NoteCreated: {
      const { note_id, title, body_md, content_hash, source_ref } =
        event.payload;
    ...
    case NOTE_EVENT_TYPES.NotePatched: {
      const { note_id, body_md, content_hash } = event.payload;
      const existing = next.notes.get(note_id);
      if (!existing) break;
