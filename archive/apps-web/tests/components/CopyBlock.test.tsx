...
describe("CopyBlock", () => {
  const mockPackWithoutPlugs: CatalogEntryExtended = {
  ...
  const mockPackWithPlugs: CatalogEntryExtended = {
    ...mockPackWithoutPlugs,
    has_plugs: true,
  ...
  it("should handle empty plug values gracefully", async () => {
    ...
    // Command should still be valid (plug set command omitted for empty values)
    const copyButton = screen.getByRole("button", {
      name: /Copy install command to clipboard/i,
    });
    await user.click(copyButton);
    ...
    await waitFor(() => {
      expect(clipboardText).toContain("aligntrue add");
      // Empty plug should not generate set command
