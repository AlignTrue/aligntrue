...
- `aligntrue remove link` now honors `--yes` for non-interactive runs and respects `--config` when selecting the config file
...
- Cursor exporter and docs now consistently preserve source filenames (e.g., `testing.md` → `testing.mdc`); removed legacy `aligntrue.mdc` references from code, tests, and docs to avoid merge-era confusion
